...
export const settings = (async () => {
  let settings = {
    ...
    skybox: window.matchMedia("(prefers-color-scheme: dark)").matches
      ? "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABAQMAAAAl21bKAAAAA1BMVEU4ODiyn42XAAAACklEQVQI" + "12NgAAAAAgAB4iG8MwAAAABJRU5ErkJggg=="
      : "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABAQMAAAAl21bKAAAAA1BMVEW6urpaLVq8AAAACklEQVQI" + "12NgAAAAAgAB4iG8MwAAAABJRU5ErkJggg==",

    // Playground settings
    pg_code: "", // Automatically loaded and executed code for the playground
    pg_version: "stable", // Set build123d version: "X.Y.Z" from PyPI ("stable" points to latest), or GitHub ref (branch/tag/commit/pr, e.g. "dev" or "pull/123/head")
    pg_debug: false, // Use debug wheels (prefer newest .dev OCP WASM wheel versions instead of stable releases)
    pg_constraints: "", // Global package constraints applied to every install. Accepted formats: JSON mapping ({"numpy": "<2"}), newline-separated constraints (numpy<2\npackaging>=24), or path to constraints file (/path/to/constraints.txt)
  ...
  for (let i = 0; i < settings.preload.length; i++) {
    ...
    if (url === "<auto>") {
      if (settings.pg_code != "") {
        // <auto> means no preload URL if code is set
        settings.preload = settings.preload.slice(0, i).concat(settings.preload.slice(i + 1));
        continue; // Skip this preload URL
      }
