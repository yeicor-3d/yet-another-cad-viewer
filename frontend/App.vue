...
networkMgr.addEventListener("update-early", (e) => viewer.value?.onProgress?.((e as CustomEvent<Array<any>>).detail.length * 0.01));
...
(async () => {
    ...
    if (sett.preload.length > 0) {
        ...
        for (let model of sett.preload) {
            ...
            networkMgr
                .load(model)
                .then(removeFromPreloadingModels)
                .catch((e) => {
...
async function loadModelManual() {
    const modelUrl = prompt(
        "For an improved experience in viewing CAD/GLTF models with automatic updates, it's recommended to use the official yacv_server Python package. This ensures seamless serving of models and automatic updates.\n\nOtherwise, enter the URL of the model to load:",
    );
            ...
            <div v-else style="height: 100%; overflow-y: auto">
                ...
                <span v-if="preloadingModels.length > 0" class="d-block text-center my-16">
                    ...
                    <span class="d-block text-center" v-for="(model, index) in preloadingModels" :key="index">
                        <template v-if="model !== undefined"> {{ model }}<span v-if="index < preloadingModels.length - 1">, </span> </template>
                    </span>
                </span>
            </div>
        </v-main>
