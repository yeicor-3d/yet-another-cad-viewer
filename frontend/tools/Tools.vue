...
import { VBtn, VCard, VCardText, VDialog, VDivider, VSpacer, VToolbar, VToolbarTitle, VTooltip } from "vuetify/lib/components/index.mjs";
...
import { mdiClose, mdiCrosshairsGps, mdiDownload, mdiGithub, mdiLicense, mdiLightbulb, mdiProjector, mdiScriptTextPlay } from "@mdi/js";
// @ts-expect-error
...
function syncOrthoCamera(force: boolean) {
    ...
    if (force || (perspectiveCam && scene.camera != perspectiveCam)) {
        ...
        let perspectiveWidthAtCenter = 2 * Math.tan((perspectiveCam.fov * Math.PI) / 180 / 2) * perspectiveCam.position.distanceTo(lookAtCenter);
...
<template>
    ...
    <v-divider />
    <h5>Camera</h5>
    <v-btn icon @click="toggleProjection"
        ><span class="icon-detail">{{ toggleProjectionText }}</span>
        <v-tooltip activator="parent"
            >Toggle (P)rojection<br />(currently {{ toggleProjectionText === "PERSP" ? "perspective" : "orthographic" }})
        </v-tooltip>
    ...
    <span>
        <v-tooltip activator="parent"
            >To rotate the light hold shift and drag the mouse or use two fingers<br />
