...
# Check if the frontend directory exists AND has actual content (not just subdirectories).
# The launcher may create yacv_server/frontend/yacv_wheel/ which makes the directory exist
# but without the actual frontend build files. In that case, fall back to dist/.
if not os.path.exists(FRONTEND_BASE_PATH) or not any(
    f
    for f in os.listdir(FRONTEND_BASE_PATH)
    if os.path.isfile(os.path.join(FRONTEND_BASE_PATH, f))
):
...
class HTTPHandler(SimpleHTTPRequestHandler):
    ...
    def translate_path(self, path: str) -> str:
        ...
        if self.directory:  # Ensure proper subdirectory
            ...
            if not path.startswith(base):
                self.send_error(
                    HTTPStatus.FORBIDDEN, "Path is not in the frontend directory"
                )
                return ""
        return path
    ...
    def send_head(self):
        ...
        if (
            path == UPDATES_API_PATH
            or path == "/"
            and query.get("api_updates") is not None
        ):
            return self._api_updates()
        elif (
            path.startswith(OBJECTS_API_PATH)
            or path == "/"
            and query.get("api_object") is not None
        ):
    ...
    def _api_updates(self):
        ...
        with self.yacv.frontend_lock.r_locked():
            ...
            if (
                self.yacv.shutting_down.is_set()
                and self.yacv.at_least_one_client.is_set()
            ):
                self.send_error(
            ...
            subscription = self.yacv.show_events.subscribe(
                yield_timeout=1.0
            )  # Keep-alive interval
            try:
    ...
    def _api_object(self, obj_name: str):
        ...
        self.send_header(
            "Content-Disposition", f'attachment; filename="{obj_name}.glb"'
        )
